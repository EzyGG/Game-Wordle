...
GAME_UUID = UUID("ceb98c87-4800-49d0-3333-3b1aa79ba293")
...
theme, word = wordle.Theme.DARK(), None


def define_theme(_theme):
    global theme
    if _theme in ["d", "dark", "s", "sombre", "0", "00", "b", "black", "n", "noir"]:
        theme = wordle.Theme.DARK()
    elif _theme in ["l", "light", "c", "clair", "1", "f", "ff", "w", "white", "blanc"]:
        theme = wordle.Theme.LIGHT()


def define_word(_word):
    global word
    word = None if _word.replace(" ", "") in ["", "0", "_"] else _word


if len(args) >= 2 and "-theme" in args and args.index("-theme") != len(args) - 1:
    _theme = args[args.index("-theme") + 1]
    args.pop(args.index("-theme") + 1)
    args.pop(args.index("-theme"))
    define_theme(_theme)

elif len(args) >= 2 and "-t" in args and args.index("-t") != len(args) - 1:
    _theme = args[args.index("-t") + 1]
    args.pop(args.index("-t") + 1)
    args.pop(args.index("-t"))
    define_theme(_theme)

if len(args) >= 2 and "-word" in args and args.index("-word") != len(args) - 1:
    _word = args[args.index("-word") + 1]
    args.pop(args.index("-word") + 1)
    args.pop(args.index("-word"))
    define_word(_word)

elif len(args) >= 2 and "-w" in args and args.index("-w") != len(args) - 1:
    _word = args[args.index("-w") + 1]
    args.pop(args.index("-w") + 1)
    define_word(_word)

chill = sum(1 for a in args if a in ["-n", "-no", "-no-commit", "-nc", "-no-points", "-np", "-chill", "-0"]) or word

wordle.Wordle(theme=theme, word=word, commit=not chill).start()
